...
the selected transport so peers use the same path. Each member publishes a
fresh join nonce in lobby member data; packets carry both sender and recipient
nonces, so delayed packets from either side of a rejoin are rejected. A peer is
announced to game code once its nonce is visible.
